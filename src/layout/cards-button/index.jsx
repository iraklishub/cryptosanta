...
import { Button, ChristmasCardIcon, LoadingSpinner } from '@/src/components'
...
const CardsButton = () => {
  ...
  return (
    <>
      ...
      >
        <ChristmasCardIcon />
        <span className="ml-2 capitalize">Holiday Cards</span>
      </Button>

      {isOpen && (
        <div className="w-full max-h-fit min-h-full bg-slate-900/40 absolute top-0 left-0 flex items-center justify-center">
          <CardsForm cssTranslate={cssTranslate} isGrinch={isGrinch} onExit={exitForm} />
        </div>
      )}
    </>
  )
